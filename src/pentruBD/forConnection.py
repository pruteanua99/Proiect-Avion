...
from api.model.modelAvion import Avion
from utility.Decorator import singleton
from api.storage.base import StorageObject
from api.model.modelGate import Gate
from api.model.modelHistory import History
from api.model.modelRunway import runway
...
class airportBD(StorageObject):
    ...
    def historyRegister(self, avion: object, poarta: object) -> None:
        try:
            table = sqlal.Table('avioane', sqlal.MetaData(),
                                autoload_with=self.engine)
            sql = sqlal.select(table.columns.Avion_NrIdentificare).where(
                table.columns.Avion_NrIdentificare == avion.IdNum.upper())
            result = self.conn.execute(sql)
            if result.first() is None:
                sql = sqlal.insert(table).values(
                    Avion_NrIdentificare=avion.IdNum.upper(), Avion_Model=avion.Model)
                self.conn.execute(sql)
                self.conn.commit()
            table = sqlal.Table('istoric', sqlal.MetaData(),
                                autoload_with=self.engine)
    ...
    def insertAvionBD(self, avion: object) -> bool:
        try:
            ...
            if result.first() is None:
                sql = sqlal.insert(table).values(
                    Avion_NrIdentificare=avion.IdNum, Avion_Model=avion.Model)
                self.conn.execute(sql)
                self.conn.commit()
                return True
            else:
    ...
    def plecareAvion(self, avion: object) -> None:
        try:
            table = sqlal.Table('istoric', sqlal.MetaData(),
                                autoload_with=self.engine)
            sql = sqlal.select(table.columns).where(
                table.columns.Avion_Id == avion.IdNum.upper()).order_by(table.columns.Istoric_Id.desc())
            result = self.conn.execute(sql)
            row = result.fetchone()
            istoric_id = row[0]
    ...
    def get_all_planes(self) -> List[Avion]:
        ...
        try:
            ...
            for row in result:
                ...
                numeAvion = row[1]
                modell = row[2]
                avion = Avion(avion_id=idUnic,
                              nrIdentificare=numeAvion, model=modell)
    ...
    def get_plane_by_NrId(self, avion_Id: int) -> List[Avion]:
        ...
        try:
            table = sqlal.Table('avioane', sqlal.MetaData(),
                                autoload_with=self.engine)
            sql = sqlal.select(table.columns).where(
                table.columns.Avion_Id == avion_Id)
            result = self.conn.execute(sql)
            for row in result:
                idUnic = row[0]
                numeAvion = row[1]
    ...
    def put_plane(self, avion: Avion) -> List[Avion]:
        ...
        try:
            table = sqlal.Table('avioane', sqlal.MetaData(),
                                autoload_with=self.engine)
            sql = sqlal.insert(table).values(
                Avion_NrIdentificare=avion.nrIdentificare, Avion_Model=avion.model)
            self.conn.execute(sql)
            self.conn.commit()
            self.listaAvioane.append(avion)
        except Exception as e:
    ...
    def put_runway(self, pista: runway) -> List[runway]:
        ...
        try:
            ...
            conn = engine.connect()
            table = sqlal.Table('piste', sqlal.MetaData(),
    ...
    def get_runway_by_id(self, runway_id: int) -> List[runway]:
        ...
        try:
            table = sqlal.Table('piste', sqlal.MetaData(),
                                autoload_with=self.engine)
            sql = sqlal.select(table.columns).where(
                table.columns.Pista_Id == runway_id)
            result = self.conn.execute(sql)
            for row in result:
                idUnic = row[0]
                pista = runway(id_runway=idUnic)
    ...
    def get_gates(self,) -> List[Gate]:
        ...
        try:
            table = sqlal.Table('porti', sqlal.MetaData(),
                                autoload_with=self.engine)
            sql = sqlal.select(table.columns)
            result = self.conn.execute(sql)
            for row in result:
    ...
    def get_gate_by_Id(self, gate_id: int) -> List[Gate]:
        ...
        try:
            table = sqlal.Table('porti', sqlal.MetaData(),
                                autoload_with=self.engine)
            sql = sqlal.select(table.columns).where(
                table.columns.Poarta_Referinta == gate_id)
            result = self.conn.execute(sql)
            for row in result:
                idUnic = row[1]
                descriere = row[0]
    ...
    def put_gate(self, poarta: Gate) -> List[Gate]:
        ...
        try:
            table = sqlal.Table('porti', sqlal.MetaData(),
                                autoload_with=self.engine)
            sql = sqlal.insert(table).values(
                Poarta_Fel=poarta.Poarta_fel, Poarta_Referinta=poarta.Poarta_Referinta)
            self.conn.execute(sql)
            self.conn.commit()
            self.listaPorti.append(poarta)
        except Exception as e:
    ...
    def update_gate(self, gate_id: int, poarta: Gate) -> List[Gate]:
        ...
        try:
            table = sqlal.Table('porti', sqlal.MetaData(),
                                autoload_with=self.engine)
            sql = sqlal.update(table).where(
                table.columns.Poarta_Referinta == gate_id).values(Poarta_Fel=poarta.Poarta_fel)
            self.conn.execute(sql)
            self.conn.commit()
            self.listaPorti.append(poarta)
        except Exception as e:
    ...
    def get_history_by_NrId(self, nrIdentificare: str) -> List[History]:
        ...
        try:
            table = sqlal.Table('istoric', sqlal.MetaData(),
                                autoload_with=self.engine)
            sql = sqlal.select(table.columns).where(
                table.columns.Avion_Id == nrIdentificare)
            result = self.conn.execute(sql)
            for row in result:
                istoric_Id = row[0]
                numeAvion = row[1]
